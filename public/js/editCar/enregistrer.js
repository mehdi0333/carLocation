...
// ! my code
document
  .getElementById("gotoSecondPage")
  .addEventListener("submit", async (e) => {
    e.preventDefault();
    const carId = e.target.getAttribute("data-carId");
    const description = document.getElementById("description").value || null;
    const marque = document.getElementById("marque").value || null;
    ...
    try {
      const response = await fetch(`/edit/car-information`, {
        ...
        body: JSON.stringify(vehiculeInformation),
      });
      if (response.ok)
        window.location.href = "/edit/enregistrer-document/" + carId;
    } catch (error) {
